              ...
              <tbody>
                {purchases.map((lista) => (
                  <tr key={lista.id} className="border-b">
                    <td className="p-2 text-sm whitespace-nowrap">{lista.id.substring(0, 8)}</td>
                    <td className="p-2 text-sm font-medium text-blue-600 whitespace-nowrap">{lista.name}</td>
                    <td className="p-2 text-sm whitespace-nowrap">
                      {lista.startDate && format(parseISO(lista.startDate), "dd/MM/yyyy", { locale: ptBR })}
                    </td>
                    <td className="p-2 text-sm whitespace-nowrap">
                      {lista.endDate && format(parseISO(lista.endDate), "dd/MM/yyyy", { locale: ptBR })}
                    </td>
                  </tr>
                ))}
              </tbody>
